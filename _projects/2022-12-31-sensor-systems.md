...
[High levels of air pollution are harmful to health](https://doi.org/10.1016/S2542-5196(20)30272-2) and thus need to be monitored. However, measurement devices are generally expensive and therefore sparsely distributed spatially. [Meanwhile, research has shown that air pollution can vary widely on small scales](https://doi.org/10.1021/acs.est.7b00891), so personal exposure at the current coverage in cities is unknown. In some parts of the world there is even a lack of access. The hope is that such gaps can be filled by means of (electrochemical) low-cost gas sensor systems (and networks). Pollutants diffuse into an electrochemical cell and are reduced or oxidized, thereby generating electrical current (Fig. 1). This current is proportional to the ambient concentration as more pollutants diffuse into the cell and engage in electrochemical reactions, which is the underlying measurement principle. Nonetheless, the low gas sensor price comes at the expense of lower reliability. Cross-sensitivities (i.e., a lack of selectivity), interferences with environmental factors such as temperature and humidity, high unit-to-unit variability, sensor aging, and concept drift are common deficiencies. In reality, the current is not only a function of one but many factors. Typically, these systems are calibrated using field data and machine learning algorithms (e.g., random forests or neural networks) to overcome some of the shortcomings.
